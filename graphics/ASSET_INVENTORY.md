...
| Kiosk decorative background | `graphics/branding/kiosk-background-tile.svg` (+ rendered `kiosk-background-tile.png`) | A real boot test called the overall kiosk look "boring/gray" and asked for an actual colorful, playful, kid-facing visual design (DoudouLinux/Qimo4kids spirit). Sparse, soft-pastel scattered shapes (stars, clouds, dots) on a transparent background, tiled at any target screen resolution rather than a single fixed-size scene image. 400x400, not the original 200x200 - an independent review rendered a realistic 4x4 composite of the first version and found visible column/diagonal alignment that no amount of repositioning shapes inside a small tile can actually fix (CSS/QSS background-repeat always recurs at a fixed pixel interval); the real fix was a larger tile with a longer repeat period, verified by re-rendering a 2x2 (800x800) composite of the new version. Deliberately sparse/low-contrast-with-itself either way - kept as gentle decoration behind the much higher-contrast app-grid buttons, not a busy texture, avoiding any seizure-risk/visual-noise pattern on a screen a toddler looks at for extended periods (confirmed via actual pixel-alpha/contrast measurement, not just asserted). |
